...
class trsf:
    ...
    def __init__(self, img_path, cutup_img_size=250, method='cython', 
                 ...
                 pboperation='divide',lower_cut_threshold=3,bg_type='radio',bg=None):
...
Topological Radio Source Finder.
        ...
        for num, img in tqdm(enumerate(self.Cutouts),total=len(self.Cutouts),desc='Cutouts Completed'):
            ...
            if img.sum() == 0:
                ...
                local_bg, sigma = self._background_estimate(img,self.bg_type)
                if self.bg != None:
                    local_bg = self.bg
                    sigma = 1
                if self.sigma != None:
        ...
        if self.sum_plot == True:
            self._summary_plots()
        # remove sources with size 1pixel
        if self.remove_1pixel == True:
            # is area is 1 pixel, remove
            self.catalogue = self.catalogue[self.catalogue['area'] > 1]
    ...
    def _background_estimate(self,img,bg_type):
        ...
        if bg_type == 'optical':
            ...
            local_bg = mad_std(img) + np.median(img)
            sigma = 5
